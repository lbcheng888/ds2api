...
- `compat.allow_meta_agent_tools`：ClaudeCode 需要原生 Agent/subagent 时设为 `true`；普通 DeepSeek 工具调用兼容场景建议保持 `false`
- `compat.default_reasoning_effort`：thinking 模型的默认推理强度，支持 `low` / `medium` / `high` / `max`；DeepSeek V4 Pro 建议 `max`
...
- `GET /admin/queue/status` 返回实时并发状态，`account_health` 会列出正在冷却的账号和剩余秒数
...
- `GET /admin/dev/diagnostics`：诊断中心数据源，汇总失败样本、抓包链、账号冷却与回放命令

当兼容层返回上游空输出、无可见输出、工具调用格式错误等错误时，响应头会尽量附带：

- `X-Ds2-Capture-Chain`：可直接传给 `/admin/dev/raw-samples/save` 的 `chain_key`
- `X-Ds2-Capture-Ids`：本次链路内的抓包 ID
- `X-Ds2-Capture-Save`：保存抓包链的接口提示
- `X-Ds2-Failure-Sample-Id`：已自动落盘的失败回放样本 ID
