...
- 对流式 Chat Completions / Responses，兼容层在当前请求暴露工具时会延迟释放工具必需回合或疑似工具前承诺文本。最新用户已经要求执行/继续/完成/本地诊断时，即使模型只输出“好的”这类普通正文，也会先被 hold，直到出现真实工具调用或最终返回 `upstream_missing_tool_call`。其他典型承诺文本如 “Let me first understand the current state by examining key files”、`In plan mode - before writing any code - I first need to understand ...`、`recap: ... 下一步：读完 ... 后写 plan 文件`、`直接切入正题。先整体扫一遍当前状态。` 或 “Now let me check the current working tree state and build/test status” 也会先被缓冲；如果本轮结束时没有真实 `tool_calls`，则输出 `upstream_missing_tool_call`，而不是把正文流给客户端后正常 `stop`。这类流式预判只做非记录型判定，首次命中后缓存 hold 状态，不会每个 chunk 都增长 failure 计数或反复做完整缺工具检测。如果本轮触发了 `current_input_file`，Chat / Responses 流式还会暂存工具前正文；一旦后续出现真实工具调用，暂存正文会被丢弃，避免先把附件里的旧 assistant 回复流给客户端。
