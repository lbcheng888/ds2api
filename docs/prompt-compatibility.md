...
- ClaudeCode Team Agents 场景中，如果 `allow_meta_agent_tools=true` 且当前请求真实暴露了 `Agent` 工具，最终收尾会先进入统一 Agent 调度器，再把明确的“启动/发起/调用/准备 N 个子代理”承诺修复成结构化 `Agent` tool_use。一轮后台 Agent 上限固定为 4；中文数词和倒装表达也会被识别，例如“准备三个子代理”“三个同时启动”“同时启动三个代理”“分三路”。若文本里列出“一个改 A、一个改 B、一个研究 C”这类具体分工，兼容层会把这些分工写入对应 Agent 的 `description` / `prompt`，而不是只返回可见承诺文本。若最新用户请求或启动文本明确要求“推进/实现/修复/改/完成”，合成的 Agent prompt 会要求子代理直接编辑文件并报告改动；纯“评估/分析/审查”请求仍保持只读。若同一最近用户回合已经有后台 Agent 启动或运行证据，且最新用户没有要求“再启动/追加”，调度器不会重复合成 Agent。
- 同一批最终工具调用会在出口层去重：`Agent` 按规范化后的任务描述去重，普通工具按工具名和规范化参数去重；`TodoWrite` / `update_todo_list` 的 `todos` 数组和 `update_plan` / `functions.update_plan` 的 `plan` 数组都会按任务文本去重，并删除已完成项；如果同名任务同时以 `pending` 和 `completed` 出现，`completed` 会压掉 stale pending。这样可避免 DeepSeek 在同一响应里重复启动相同子代理、重复执行同一条命令，或让客户端任务列表显示重复/已完成任务。`EnterPlanMode` / `switch_mode(plan)` 属于客户端模式切换，不算实现进展；当最新用户要求继续、执行、修复、实现、验证或完成时，兼容层会在 Chat / Responses / Claude realtime 的最终文本路径和已解析 tool_calls 出口同时拦截这类进入计划模式调用，并返回 `upstream_missing_tool_call`，避免 ClaudeCode 连续显示 `Entered plan mode`。
- `/admin/dev/diagnostics` 的 `harness_metrics` 按 client profile 暴露兼容层观测：`repairs` 记录最终输出修复命中，`streams` 记录流式工具块处理，`failures` 记录缺工具/坏工具拦截，`dedupes` 用于记录已丢弃的重复工具调用或重复任务项数量；当前约定的 dedupe reason 包括 `tool_calls` 和 `todo_items`。
- 对 ClaudeCode/OpenAI Chat / Responses 以及 Claude `/v1/messages` realtime 流的最终收尾，兼容层会先编译当前请求意图，并用执行证据层核对最终文本。它会拦截“现在直接动手 / 把代码写到位 / 现在实现 / 现在修改 / 现在写”这类中文执行承诺，也会拦截 `In plan mode` / `before writing any code` 包裹的“先理解/读文件/consult lessons”承诺，以及 `recap: ... 下一步：读完/确认/写 plan 文件` 这类续写承诺。它还会拦截“已修改 / 测试通过 / 已启动 N 个子代理”但当前回合没有对应 Edit/Write、Bash/test 或 Agent 证据的完成宣称。对于最新用户请求本身已经明确要求执行、继续、完成、修复、验证、重启或本地诊断的回合，只要当前请求有 Read / Search / Edit / Write / Bash / Agent 这类可执行工作工具，且本轮没有任何真实工作工具证据，就返回 `upstream_missing_tool_call`；纯解释、纯方案、只分析或显式不用工具的请求不会触发这个强制工具闸门。
- 如果最终文本只是“Let me read/examine current codebase/source files...”这类泛化安全读探测承诺，且没有写文件、编辑、运行命令、启动 Agent 或完成宣称，兼容层会在缺工具错误前合成一个真实只读探索工具调用：优先用 `Bash` 执行 `pwd && find . -maxdepth 2 -type f | sed 's#^\./##' | sort | head -200`，否则按可用工具退到 `List` / `Glob` / 明确路径 `Read`。具体符号搜索、报错定位、实现/修复/测试/重启/提交/启动代理这类动作不做合成，仍走缺工具重试或错误暴露。
- 执行证据不是只看最后一段文本。兼容层会按最新 user turn 建立执行账本，历史窗口里的真实 Read / Search / Edit / Write / Bash / Agent / TaskOutput 才算证据；TaskCreate / TaskUpdate / TodoWrite / TodoRead 这类 UI 任务工具不算真实执行。这样可以避免“只建任务列表就停”被误判为已执行，也能避免跨轮工具证据丢失后重复拦截。
- 对有结构的实质分析报告或阶段性结论，如果文本里没有继续承诺读、改、跑、启动工具，也没有“已修改 / 测试通过 / 已启动 Agent”这类缺证据完成宣称，兼容层不再把它误判为 `upstream_missing_tool_call`。这条规则适用于纯分析/方案类请求，也适用于已经有后台 Agent 启动证据后的进展汇总；但最新用户明确要求执行、继续、完成、修复、验证或本地诊断时，仍然必须有真实工作工具证据。低信息等待文本或空文本则会优先合成 `TaskOutput` 轮询仍在运行的 Agent。
- 最终工具调用会先通过统一事务内核：final repair、工具解析、schema defaults、TaskOutput id 校验、schema/meta 参数归一化、同批去重都会在同一个入口完成。OpenAI Chat、OpenAI Responses 和 Claude realtime 流出口在返回给客户端前都会执行最终工具闸门，确保重复工具、任务追踪-only 工具和只说不做文本不会穿透到客户端。
- 对同一当前任务窗口里的重复探索，兼容层会在工具调用发给客户端前阻断并返回 `upstream_repeated_exploration`。检测范围同时覆盖 live prompt 和 `DS2API_HISTORY.txt` transcript 中最新 user turn 之后的 assistant 工具历史。当前阻断范围包括重复 Read / read_file、Grep / Search / Glob，只读型 `rg` / `grep` / `find` / `ls`，以及只读 `git status` / `git diff` / `git log` / `git show` / `git rev-parse` / `git ls-files`；链式 shell 里的 `echo` 分隔段会被忽略，`2>/dev/null` 和 `2>&1` 这类空输出重定向不影响去重，`git -C /path status` 和当前 cwd 下的 `git status` 也会归并为同一类状态检查。`go test`、build、`git add` / `git commit` 和其他写入型 shell 不按探索重复处理。
- 对 ClaudeCode 的 `Edit` / `Update` / `MultiEdit`，如果当前任务窗口里同一文件最近一次编辑工具结果是 `Error editing file`，且失败后还没有重新 `Read` 该文件，兼容层会把下一次同文件编辑重试改成 `Read`。这样强制模型拿最新文件内容重建 `old_string`，避免连续把同一 stale patch 发给客户端并刷出多次编辑错误。
- 对同一批最终工具调用中的 `Bash` / shell 类工具，兼容层会串行化放行：一批里只保留第一条 shell 命令，其余 shell 命令丢弃并计入 `tool_calls` dedupe 指标，非 shell 工具保留。ClaudeCode 会并发执行同批 tool_calls，任一 Bash 非零退出会把同批其他 Bash 标记为 `Cancelled: parallel tool call ... errored`；串行化后最多只有一条 shell 命令进入客户端，后续检查必须等观察结果后再发下一轮。
- 对流式 Chat Completions / Responses，兼容层在当前请求暴露工具时会先缓存普通正文，不直接流给客户端；如果后续出现真实 `tool_calls`，这些工具前正文会被丢弃，只输出工具调用；如果本轮最终没有工具且通过收尾校验，才把缓存正文释放为正常文本。最新用户已经要求执行/继续/完成/本地诊断时，即使模型只输出“好的”这类普通正文，也会先被 hold，直到出现真实工具调用或最终返回 `upstream_missing_tool_call`。其他典型承诺文本如 “Let me first understand the current state by examining key files”、`In plan mode - before writing any code - I first need to understand ...`、`recap: ... 下一步：读完 ... 后写 plan 文件`、`直接切入正题。先整体扫一遍当前状态。` 或 “Now let me check the current working tree state and build/test status” 也不会提前可见；如果本轮结束时没有真实 `tool_calls`，则输出 `upstream_missing_tool_call`，而不是把正文流给客户端后正常 `stop`。这类流式预判只做非记录型判定，首次命中后缓存 hold 状态，不会每个 chunk 都增长 failure 计数或反复做完整缺工具检测。
- Claude `/v1/messages` realtime 流也会在工具模式下抑制裸 DSML/XML 工具语法碎片；即使上游输出半截 `<|DSML|tool_calls>` 或畸形闭合标签，兼容层也不会把这些协议文本作为可见正文发给 ClaudeCode。
- 如果上游把 DeepSeek prompt 分隔符 `<｜end▁of▁sentence｜>` / `<| end_of_sentence |>` 泄漏到 assistant 输出，兼容层把它视为硬结束边界：非流式直接截断该标记后的内容，流式会缓冲可能被 SSE 拆开的特殊 token 前缀，一旦拼成完整标记就丢弃本轮后续可见文本和工具候选；OpenAI Chat / Responses 与 Claude realtime 的工具筛分器也只接收清洗后的文本，避免把下一段 prompt、源码片段或历史 transcript 当成正常回复或工具候选继续透给客户端。
- OpenAI Chat / Responses / Claude direct 的空回复和缺工具错误暴露前会默认做一次内部补偿重试：第一次上游完整结束后，如果最终可见正文为空、没有解析到工具调用、也没有已经向客户端流式发出工具调用，并且终止原因不是 `content_filter`，兼容层会复用同一个 `chat_session_id`、账号、token 与工具策略，把原始 completion `prompt` 追加固定空回复后缀后重新提交一次；如果最终被判为 `upstream_missing_tool_call`，则改追加固定缺工具后缀，要求模型立刻输出真实工具调用而不是继续解释下一步。重试遵循 DeepSeek 多轮对话协议：从第一次上游 SSE 流中提取 `response_message_id`，并在重试 payload 中设置 `parent_message_id` 为该值，使重试成为同一会话的后续轮次而非断裂的根消息；同时重新获取一次 PoW（若 PoW 获取失败则回退到原始 PoW）。该重试不会重新标准化消息、不会新建 session、不会切换账号，也不会向流式客户端插入重试标记；OpenAI 流式第二次 thinking / reasoning 会按正常增量直接接到第一次之后，并继续使用 overlap trim 去重；Claude direct 流式只有在缺工具正文尚未发给客户端时才延迟失败并重试。若第二次仍为空或仍缺工具，终端错误码分别保持 `upstream_empty_output` / `upstream_missing_tool_call`；若任一尝试触发空 `content_filter`，不做补偿重试并保持 `content_filter` 错误。JS Vercel 运行时同样设置 `parent_message_id`，但因无法直接调用 PoW API 而复用原始 PoW。
- DeepSeek Web 有时不会返回结构化 `code=content_filter` 或 `CONTENT_FILTER` 状态，而是把中文安全拒答模板直接放进 `response/content`。兼容层会把“系统检测到当前输入存在敏感内容 / 无法响应您的请求 / 更换其他合规话题”这类完整拒答识别为 `content_filter` 终态，并丢弃可见拒答正文，避免客户端把它当作正常回答继续等待工具执行。
...
在 assistant 最终回包阶段，如果某个 tool 参数在声明 schema 中明确是 `string`，兼容层会在把解析后的 `tool_calls` / `function_call` 重新序列化成 OpenAI / Responses / Claude 可见参数前，递归把该路径上的 number / bool / object / array 统一转成字符串；其中 object / array 会压成紧凑 JSON 字符串。这个保护只对 schema 明确声明为 string 的路径生效，不会改写本来就是 `number` / `boolean` / `object` / `array` 的参数。对 schema 明确声明为 `array` 的路径，兼容层只做确定性数组还原：已是数组则保留，`item` 包装、单个符合 item schema 的对象、以及可解析的 JSON/XML 对象列表会还原成数组；普通不可解析文本不会被硬凑成数组。这样可以兼容 DeepSeek 输出了结构化片段、但上游客户端工具 schema 又严格要求字符串或数组参数的场景（例如 `content`、`prompt`、`path`、`taskId`、`todos` 等）。
...
- `current_input_file` 默认开启；它用于把“完整上下文”合并进 `DS2API_HISTORY.txt` 上下文文件。当最新 user turn 的纯文本长度达到 `current_input_file.min_chars`（默认 `12000`，配置为 `0` 时也按默认值处理；如需几乎总是触发可设为 `1`）时，兼容层会上传一个文件名为 `DS2API_HISTORY.txt` 的上下文文件。文件内容会先做 OpenAI 消息标准化，再序列化成按轮次编号的 `DS2API_HISTORY.txt` 风格 transcript，带有 `# DS2API_HISTORY.txt` 标题、`metadata` 索引和 `=== N. ROLE ===` 分段；live prompt 中会保留 continuation 指令、显式重复最新 user request，并内联最近 assistant/tool 热上下文和执行账本，避免网页端模型只盯着附件里的上一条 assistant 回复而复读旧内容。
...
- 同账号、同模型类型、同完整上下文内容在短 TTL 内会复用已经上传的 `DS2API_HISTORY.txt` file id，避免每轮重复上传完全相同的大文件；缓存键不保存明文 token，只使用 token/account 的稳定摘要。
