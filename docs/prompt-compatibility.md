...
- managed-account 模式会按 conversation / thread / session 标识做账号亲和：兼容层会从 `X-Ds2-Conversation-ID`、`X-Ds2-Conversation-Key`、`X-Ds2-Session-ID`、`X-Conversation-ID`、`X-Thread-ID`、`X-Session-ID`，同名 query 参数，以及请求体/`metadata` 中的 `conversation_id` / `thread_id` / `session_id` 等字段提取 affinity key。命中后同一调用方的同一会话会优先续租同一个 DeepSeek 账号，默认 TTL 为 3600 秒，可通过 `runtime.account_affinity_ttl_seconds` 或 `DS2API_ACCOUNT_AFFINITY_TTL_SECONDS` 调整。账号失败并完成 failover 后，亲和关系会迁移到新账号；没有稳定会话标识时仍按账号池正常调度。Claude Code 如果没有显式会话标识，兼容层会用首个足够长的 user 任务内容、模型和工作目录提示派生一个保守 affinity key；过短的“继续”类请求不会派生，避免不同任务误黏。
...
对文件编辑类工具，提示词会额外要求 `Edit` / `Update` / `MultiEdit` 的 `old_string` 必须来自最新一次 `Read`，并带足可唯一定位的上下文；如果编辑失败或同一文件刚被其他编辑改过，必须重新读取文件再重试，避免客户端侧出现 `Error editing file`。
...
- `ref_file_ids` 是 DeepSeek 账号侧资源，不能跨账号复用；OpenAI Chat / Responses 的 managed-account failover 只有在当前请求没有 `ref_file_ids` 时才会透明切到下一个账号。若请求包含 inline upload、附件或 current input file 产生的文件引用，兼容层会保留当前账号并返回该账号上的错误，避免把 A 账号文件 ID 发送给 B 账号导致上下文错乱。
