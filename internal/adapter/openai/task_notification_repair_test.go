...
func TestSynthesizeTaskOutputToolCallsFromTaskNotification(t *testing.T) {
	...
	got := synthesizeTaskOutputToolCallsFromTaskNotification(prompt, []string{"Read", "TaskOutput"}, true)
	if len(got) != 1 {
...
func TestSynthesizeTaskOutputRequiresMetaAgentToolsAllowed(t *testing.T) {
	...
	got := synthesizeTaskOutputToolCallsFromTaskNotification(prompt, []string{"TaskOutput"}, false)
	if len(got) != 0 {
