...
async function handleVercelStream(req, res, rawBody, payload) {
  ...
  try {
    ...
    const failTerminal = async (status, message, code) => {
      if (ended) {
        return true;
      }
      ended = true;
      sendFailedChunk(res, status, message, code);
      await releaseLease();
    ...
    const finish = async (reason, options = {}) => {
      ...
      if (detected.length > 0 && !toolCallsDoneEmitted) {
        ...
        for (const evt of tailEvents) {
          if (evt.type === 'error') {
            return await failTerminal(502, evt.message || 'Upstream model emitted invalid tool call syntax.', evt.code || 'upstream_invalid_tool_call');
          }
              ...
              for (const p of parsed.parts) {
                ...
                if (p.type === 'thinking') {
                  ...
                  for (const evt of events) {
                    if (evt.type === 'error') {
                      return { terminal: await failTerminal(502, evt.message || 'Upstream model emitted invalid tool call syntax.', evt.code || 'upstream_invalid_tool_call'), retryable: false };
                    }
