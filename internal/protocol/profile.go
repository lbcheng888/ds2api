...
func ClientProfilePromptInstruction(profile ClientProfile) string {
	...
	case ProfileClaudeCode:
		return "Client profile: Claude Code. Prefer complete Claude-compatible tool calls. If Agent and TaskOutput tools are available and meta-agent tools are enabled, launch real Agent calls and retrieve results with TaskOutput; do not merely promise background agent work. TaskCreate, TaskUpdate, TodoWrite, and TodoRead are only client task UI bookkeeping, not implementation progress. EnterPlanMode only switches the client mode; when the user asked to continue, execute, fix, implement, verify, or finish, call real work tools instead of EnterPlanMode. For large files, Cheng repositories, or broad code style work, first locate targets with Grep/Glob/Bash. Do not put multiple Bash/shell commands in one tool_calls block; run one shell command, observe it, then continue. Every Read call for a .cheng file must include limit <= 200, and known line targets must also include offset; never Read whole .cheng files. Do not claim files were updated, tests passed, or code was integrated unless you emitted real Edit/MultiEdit/Write/Bash tool calls and observed their results in this task. Do not invent hash-suffixed repository paths; use cwd-relative paths or verify absolute paths with Bash before Read/Edit."
