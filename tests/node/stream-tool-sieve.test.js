...
test('sieve reports incomplete captured XML tool blocks as invalid syntax', () => {
  const events = runSieve(
    [
      '前置正文G。',
      '<tool_calls>\n',
  ...
  const leakedText = collectText(events);
  const errors = events.filter((evt) => evt.type === 'error');
  const hasToolCalls = events.some((evt) => evt.type === 'tool_calls' && evt.calls?.length > 0);
  assert.equal(hasToolCalls, false);
  assert.equal(leakedText, '前置正文G。');
  assert.equal(errors.length, 1);
  assert.equal(errors[0].code, 'upstream_invalid_tool_call');
});
